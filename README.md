...
```gradle 
  implementation 'com.trackier:android-sdk:1.6.52' // Add this Trackier SDK dependency in the build.gradle
```
...
```
#### Hiding the secret key using NDK. Just follow the below steps for hiding secret key
...
```
./gradlew hideSecret -Pkey=randomKey -PkeyName=secretKey
```
...
```
val secretKey = Secrets().getYoutubeKey(packageName)
```
